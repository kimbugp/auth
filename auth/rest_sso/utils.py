...
from . import claims
from .keys import get_private_key_and_key_id, get_public_key_and_key_id
...
def create_session_payload(session_token, user, **kwargs):
    return {
        ...
        claims.SESSION_ID: str(session_token.pk),
...
def encode_jwt_token(payload):
    if payload.get(claims.TOKEN) not in (
    ...
    if not payload.get(claims.AUDIENCE):
        if (
            payload.get(claims.TOKEN) == claims.TOKEN_SESSION
            and api_settings.SESSION_AUDIENCE is not None
        ):
            payload[claims.AUDIENCE] = api_settings.SESSION_AUDIENCE
        elif (
            payload.get(claims.TOKEN) == claims.TOKEN_AUTHORIZATION
            and api_settings.AUTHORIZATION_AUDIENCE is not None
        ):
    ...
    if not payload.get(claims.EXPIRATION_TIME):
        if (
            payload.get(claims.TOKEN) == claims.TOKEN_SESSION
            and api_settings.SESSION_EXPIRATION is not None
        ):
            payload[claims.EXPIRATION_TIME] = (
                datetime.utcnow() + api_settings.SESSION_EXPIRATION
            )
        elif (
            payload.get(claims.TOKEN) == claims.TOKEN_AUTHORIZATION
            ...
            payload[claims.EXPIRATION_TIME] = (
                datetime.utcnow() + api_settings.AUTHORIZATION_EXPIRATION
            )
    ...
    private_key, key_id = get_private_key_and_key_id(
        issuer=payload[claims.ISSUER], key_id=session_id
    )
...
def decode_jwt_token(token):
    ...
    if (
        api_settings.ACCEPTED_ISSUERS is not None
        and unverified_issuer not in api_settings.ACCEPTED_ISSUERS
    ):
        raise InvalidIssuerError("Invalid issuer")
    public_key, key_id = get_public_key_and_key_id(
        issuer=unverified_issuer, key_id=unverified_key_id
    )
    options = {
...
def authenticate_payload(payload, request=None):
    from ..rest_sso.models import SessionToken
    ...
    if api_settings.VERIFY_SESSION_TOKEN:
        try:
            session_token = (
                ...
                .select_related("user")
                .get(
                    pk=payload.get(claims.SESSION_ID),
